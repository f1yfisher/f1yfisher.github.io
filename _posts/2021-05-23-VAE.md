...
首先来看一组数据![](http://latex.codecogs.com/svg.latex?x=\{x_1,x_2,x_3,...,x_K\})，我们要训练一个模型将数据![](http://latex.codecogs.com/svg.latex?x)转换到隐空间得到![](http://latex.codecogs.com/svg.latex?z)，为便于理解，本文只考虑输入维度与编码维度相等的情况，即![](http://latex.codecogs.com/svg.latex?z=\{z_1,z_2,z_3,...,z_K\})，模型可以表示为<img src="https://latex.codecogs.com/svg.latex?q_\phi(z|x)" title="q_\phi(z|x)" />，再将隐空间的数据解码到原始的数据空间得到重构的![](http://latex.codecogs.com/svg.latex?\widehat{x}=\{\widehat{x}_1,\widehat{x}_2,\widehat{x}_3,...,\widehat{x}_K\})，改模型可以表示为![](http://latex.codecogs.com/svg.latex?p_\theta(x|z))，之前在看博客和原文时对这个模型一直存在着比较大疑问，认为应该是![](http://latex.codecogs.com/svg.latex?p_\theta(\widehat{x}|z))，但是在博客和原文中都没有出现过这种写法，在反复对原文和博客的阅读中，理解了这种写法，在这里![](http://latex.codecogs.com/svg.latex?x)与![](http://latex.codecogs.com/svg.latex?\widehat{x})都同属于原始数据空间，![](http://latex.codecogs.com/svg.latex?x)也只是原始数据空间中的一些样本，不能表示原始空间的分布，若能直接表示原始数据空间的分布，那就不需要费劲地将原始数据转换到隐空间，再将隐空间的数据转换到原始数据空间。同时这里的模型输出的![](http://latex.codecogs.com/svg.latex?x)与![](http://latex.codecogs.com/svg.latex?z)表示的是一种分布，而最终输出的数据是对这两个分布采样得到的结果。
